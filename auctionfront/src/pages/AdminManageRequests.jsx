...
const AdminManageAuctions = () => {
  ...
  return (
    <div className="min-h-screen bg-yellow-50 px-6 py-10">
      <h1 className="text-3xl font-bold text-yellow-600 mb-6 text-center">Manage Auction Requests</h1>

      {loading ? (
        <p className="text-center text-gray-600">Loading requests...</p>
      ) : requests.length === 0 ? (
        <p className="text-center text-gray-600">No pending auction requests.</p>
      ) : (
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {requests.map((req) => (
            <div key={req._id} className="bg-white shadow-md rounded-lg overflow-hidden">
              <img
                src={req.imageUrl}
                alt={req.name}
              ...
              <div className="p-4">
                ...
                <p className="mt-2 text-yellow-700 font-semibold">Base Price: ₹{req.baseprice}</p>
                <p className="text-xs text-gray-400 mt-1">Requested by: {req.requestedby?.username || "User"}</p>
